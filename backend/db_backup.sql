...
--
-- Name: user_role; Type: TYPE; Schema: public; Owner: hunn
--
...
ALTER TYPE public.user_role OWNER TO hunn;
...
--
-- Custom role model and cleanup helpers
--

-- Add role column backed by enum and backfill from is_admin
ALTER TABLE public.users
    ADD COLUMN role public.user_role NOT NULL DEFAULT 'student';

UPDATE public.users
SET role = CASE WHEN is_admin THEN 'super_admin' ELSE 'student' END;


-- Guard teachers to correct role
CREATE OR REPLACE FUNCTION public.ensure_teacher_role() RETURNS trigger AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM public.users u
        ...
        RAISE EXCEPTION 'Teacher % must have role teacher/super_admin', NEW.teacher_id;
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trg_classes_teachers_role ON public.classes_teachers;
CREATE TRIGGER trg_classes_teachers_role
    BEFORE INSERT OR UPDATE ON public.classes_teachers
    FOR EACH ROW EXECUTE FUNCTION public.ensure_teacher_role();


-- Guard students to student role
CREATE OR REPLACE FUNCTION public.ensure_student_role() RETURNS trigger AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM public.users u
        ...
        RAISE EXCEPTION 'Student % must have role student', NEW.student_id;
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trg_courses_students_role ON public.courses_students;
CREATE TRIGGER trg_courses_students_role
    BEFORE INSERT OR UPDATE ON public.courses_students
    FOR EACH ROW EXECUTE FUNCTION public.ensure_student_role();


-- Soften deletes for admin cleanup
ALTER TABLE ONLY public.classes_teachers DROP CONSTRAINT IF EXISTS classes_teachers_class_id_fkey;
ALTER TABLE ONLY public.classes_teachers
    ADD CONSTRAINT classes_teachers_class_id_fkey FOREIGN KEY (class_id) REFERENCES public.classes(class_id) ON DELETE CASCADE;

ALTER TABLE ONLY public.classes_teachers DROP CONSTRAINT IF EXISTS classes_teachers_teacher_id_fkey;
ALTER TABLE ONLY public.classes_teachers
    ADD CONSTRAINT classes_teachers_teacher_id_fkey FOREIGN KEY (teacher_id) REFERENCES public.users(user_id) ON DELETE CASCADE;

ALTER TABLE ONLY public.courses_students DROP CONSTRAINT IF EXISTS courses_students_course_id_fkey;
ALTER TABLE ONLY public.courses_students
    ADD CONSTRAINT courses_students_course_id_fkey FOREIGN KEY (course_id) REFERENCES public.courses(course_id) ON DELETE CASCADE;

ALTER TABLE ONLY public.courses_students DROP CONSTRAINT IF EXISTS courses_students_student_id_fkey;
ALTER TABLE ONLY public.courses_students
    ADD CONSTRAINT courses_students_student_id_fkey FOREIGN KEY (student_id) REFERENCES public.users(user_id) ON DELETE CASCADE;

ALTER TABLE ONLY public.lesson_progess DROP CONSTRAINT IF EXISTS lesson_progess_lesson_id_fkey;
ALTER TABLE ONLY public.lesson_progess
    ADD CONSTRAINT lesson_progess_lesson_id_fkey FOREIGN KEY (lesson_id) REFERENCES public.lessons(lesson_id) ON DELETE CASCADE;

ALTER TABLE ONLY public.lesson_progess DROP CONSTRAINT IF EXISTS lesson_progess_student_id_fkey;
ALTER TABLE ONLY public.lesson_progess
    ADD CONSTRAINT lesson_progess_student_id_fkey FOREIGN KEY (student_id) REFERENCES public.users(user_id) ON DELETE CASCADE;

ALTER TABLE ONLY public.submissions DROP CONSTRAINT IF EXISTS submissions_assignment_id_fkey;
ALTER TABLE ONLY public.submissions
...
ALTER TABLE ONLY public.submissions DROP CONSTRAINT IF EXISTS submissions_student_id_fkey;
ALTER TABLE ONLY public.submissions
    ADD CONSTRAINT submissions_student_id_fkey FOREIGN KEY (student_id) REFERENCES public.users(user_id) ON DELETE CASCADE;

ALTER TABLE ONLY public.submissions DROP CONSTRAINT IF EXISTS submissions_grade_by_fkey;
ALTER TABLE ONLY public.submissions
    ADD CONSTRAINT submissions_grade_by_fkey FOREIGN KEY (grade_by) REFERENCES public.users(user_id) ON DELETE SET NULL;
